...
export function buildSetCookie(token: string): string {
  return `${COOKIE_NAME}=${token}; Max-Age=${MAX_AGE}; Path=/budget; HttpOnly; Secure; SameSite=Lax`;
}

export function buildClearCookie(): string {
  return `${COOKIE_NAME}=; Max-Age=0; Path=/budget; HttpOnly; Secure; SameSite=Lax`;
}
